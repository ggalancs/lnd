...
func (frs *failingRetributionStore) FailNext() {
	frs.mu.Lock()
	frs.failNext = true
	frs.mu.Unlock()
}
...
func (frs *failingRetributionStore) Add(retInfo *retributionInfo) error {
	...
	if frs.failNext == true {
		frs.failNext = false
		return errors.New("retribution store failed")
	}
...
// TestBreachHandoffFail asserts that the breach arbiter will receive.
func TestBreachHandoffFail(t *testing.T) {
	notifier := makeMockSpendNotifier()
	alice, bob, cleanUpChans, err := createTestChannelsWithNotifier(1,
		notifier)
	if err != nil {
...
func createTestArbiter(t *testing.T, notifier chainntnfs.ChainNotifier,
	...
	store := newFailingRetributionStore(restartDb)
